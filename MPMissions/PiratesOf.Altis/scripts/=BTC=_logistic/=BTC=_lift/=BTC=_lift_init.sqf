...
[] spawn
{
	...
	if (BTC_def_hud == 1) then {player addAction [("<t color=""#ED2744"">" + ("Interface An\Aus") + "</t>"),BTC_dir_action, [[],{if (BTC_Hud_Cond) then {BTC_Hud_Cond = false;} else {BTC_Hud_Cond = true;_hud = [] spawn BTC_fnc_hud;};}], -8, false, false, "", "(vehicle player) isKindOf ""Helicopter"" && driver (vehicle player) == player"];};
	...
	player addAction [("<t color=""#ED2744"">" + ("Anheben") + "</t>"),BTC_dir_action, [[],BTC_attach_cargo], 9, true, false, "", "[] call BTC_lift_check"];
	player addAction [("<t color=""#ED2744"">" + ("Loslassen") + "</t>"),BTC_dir_action, [[],BTC_detach_cargo], -9, true, false, "", "BTC_lifted == 1"];
