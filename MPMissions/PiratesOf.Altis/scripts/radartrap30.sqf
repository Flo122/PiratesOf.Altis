...
while {playerSide == civilian} do 
{
	...
	_blitzer = [Kavala_N_30,Kavala_S_30,Kavala_O_30];
	...
	{
		if ((player == driver _vehicle) && (_vehicle distance _x < 20) && (_speed >= 40)) then 
		{ 
			titleCut [" ","white in",1];
			switch(true) do
			{
			...
			hint parseText format ["Du wurdest gerade geblitzt! (Limit: 30, Geschwindigkeit: %1)", round _speed];
