class Life_my_smartphone {
	...
	class controls {
		...
		class AdminButton : Life_RscButtonMenu {
			idc = 888801;
			text = "$STR_Global_Admin";
			onButtonClick = "createDialog ""Life_smartphone_admin"";";
			x = 0.1875 + (12.65 / 40) + (1 / 250 / (safezoneW / safezoneH));
			y = 0.9 - (1 / 25);
			w = (7.45 / 40);
			h = (1 / 25);
		};
		...
		class CopGlobal : Life_RscButtonMenu {
			idc = 888800;
			text = "$STR_SMARTPHONE_COPMSG";
			onButtonClick = "createDialog ""Life_smartphone_cop"";";
...
class Life_smartphone_notruf {
	...
	class controls {
		...
		class APHReq : life_RscButtonMenu
		{
			idc = 888889;
...
class Life_smartphone_cop {
	idd = 888808;
	name= "life_my_smartphone_cop";
	movingEnable = false;
	enableSimulation = true;
	onLoad = "[9] spawn life_fnc_newMsg;";
	...
	class controls {
		...
		class CopMsgAll : life_RscButtonMenu 
		{
			...
			colorBackground[] = {0.030, 0.144, 0.255,1.0};
			onButtonClick = "[9,-1,(ctrlText 888806)] call life_fnc_newMsg;";
			x = 0.11;
			y = 0.31;
			w = 0.235;
			h = (1 / 25);
