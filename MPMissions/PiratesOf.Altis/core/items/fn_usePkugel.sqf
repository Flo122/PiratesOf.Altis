...
private["_done_struggle","_bomb"]; 
 ...
 {
	titleText ["Du betrachtest die Piratenkugel ...", "PLAIN"]; 
	sleep 3;
 }
 else
 {
	[[0,format["!! %1 will eine Piratenkugel zuenden !!", name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
 };
...
while { !_done_struggle } do 
{
	...
	else
	{
		...
		{
			titleText ["Irgendwas stimmt mit der Piratenkugel nicht!", "PLAIN"]; 		
		};	
...
{
	[[0,format["%1 konnte die Piratenkugel nicht zuenden. Er ist tot.", name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};
...
if(playerSide == civilian) then
{
	...
	[[0,format["!!! %1 hat eine Piratenkugel gezuendet !!!", name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};
