...
if(isNull _unit) exitWith {}; //if unit is null, than NO

		{
			...
			if(_val > 0 ) then
				{
				if( _str == "Skalpel") then
					{
					
					if(([false ,"Handschuhe",1] call life_fnc_handleInv)) then
					{
					...
					if(life_inv_kidney >= 2) exitWith {hint "Du kannst nicht mehr als 2 Nieren haben!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
					if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hint "Du hast nicht genug platz im Inventar!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
					...
					sleep 10;//length of action, had weird results with any higher - obv not realistic, but whatever
					...
					hint "Dir fehlen Handschue, damit du dich nicht infizierst.";
					};
					
					} else {
					hint "Du brauchst ein Skalpel um einen Körper aufzuscheiden.";
					};
				};
